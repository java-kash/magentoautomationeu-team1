...
public class ApiTestStep extends TestBase {
    ...
    public void userShouldBeAbleToCreateACustomerGroup() {

    }

    @When("User sends request for create a customer group")
    public void userSendsRequestForCreateACustomerGroup() {
        response = RestAssured.given().log().all()
                .auth().basic(username, password)
                .header("Content-Type", "application/json")
                .body(PayloadUtility.getCustomerGroupPayload("Kadirdan Test", 5))
                .when().post(baseURL + ":" + port + "/customergroup");
    }

    @Then("User should get response {int}")
    public void userShouldGetResponseStatusAndTaxClassIdNumber(int statusCode) {
